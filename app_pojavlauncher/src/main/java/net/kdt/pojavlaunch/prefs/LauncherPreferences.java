...
public class LauncherPreferences {
    ...
    public static final String PREF_MODPACK_VERSION = "0";
    ...
    private static int findBestRAMAllocation(Context ctx){
        ...
        if (deviceRam < 1024) return 444;
        if (deviceRam < 1536) return 672;
        if (deviceRam < 2048) return 984;
        // Limit the max for 32 bits devices more harshly
        if (is32BitsDevice()) return 696;

        if (deviceRam < 3064) return 1404;
        if (deviceRam < 4096) return 1716;
        if (deviceRam < 6144) return 2304;
        return 3072; //Default RAM allocation for 64 bits
    }
